...
def grade_func(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of exp
    ...
    function = 'exp'
    testcase = [(1,None,5,2.71828),
                (1,1e-12,11,2.71828182846),
                (-2,None,5,0.13534),
                (-2,1e-12,11,0.13533528324),
                (8,1e-1,0,2981.0),
                (8,None,5,2980.95799),
                (8,None,11,2980.95798664543),
                (10,1,1,22025.4)]
    ...
    if type(env) == str:
        ...
        outp.write("File %s was not properly processed.\n" % repr(file))
        return (FAIL_INCORRECT, 0)
    ...
    for pair in testcase:
        try:
            ...
            if args[1] is None:
                args = pair[:1]
            data = '(%s)' % repr(args[0]) if len(args) == 1 else repr(args)
            expected = pair[-1]
            received = func(*args)
            received = round(received,pair[2]) if type(received) == float else received
            ...
            elif type(received) != float or not introcs.isclose(received,expected):
                outp.write("The call %s%s returns %s, not %s.\n" % (function, data, repr(received), repr(expected)))
                score -= 1/len(testcase)
                return 1
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
...
def grade_file(file,outp=sys.stdout):
    ...
    if not status:
        ...
        if not status:
           status, p2b = grade_structure(file,'exp',1,outp)
        else:
            p2b = 0
        p2 = 0.8*p2a+0.2*p2b
        if p2 == 1:
