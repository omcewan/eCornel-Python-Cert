...
def check_print(env,first,last,correct=None):
    """
...
def grade_function(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the function rollem
    ...
    if not 'random.randint' in code:
        outp.write('Procedure rollem does not generate random numbers.\n')
    ...
    try:
        if env.module.rollem(FIRST,LAST) != 6:
    ...
    try:
        if env.module.rollem(FIRST,LAST) != 7:
            outp.write('Function rollem returns the wrong value.\n')
            score -= 0.35
            if not step:
                return (FAIL_INCORRECT,max(0,score))
    ...
    if len(env.printed) > 0:
        outp.write('Function rollem contains print statements.\n')
        score -= 0.2
        if not step:
            return (FAIL_INCORRECT,max(0,score))
    elif len(env.inputed) > 0:
        outp.write('Procedure rollem contains input statements.\n')
        score -= 0.1
        if not step:
            return (FAIL_INCORRECT,max(0,score))
...
def grade_spec(file,step=0,outp=sys.stdout):
    ...
    if (correct != actuals):
        ...
        score -= 0.8
        if not step:
            return (FAIL_INCORRECT,score)
    
    return (TEST_SUCCESS, score)


def grade_optional(file,step=0,outp=sys.stdout):
    """
    Returns the test result for the optional argument(s)
    
    The step parameter is the phase in the grading pass.  Step 0 only requires one
    argument to optional.  Step 1 requires both to be optional. Steps 0 and 1 will 
    ...
    try:
        if env.module.rollem(FIRST,LAST) != 14:
            outp.write('Function rollem returns the wrong value.\n')
            score -= 0.3
            if step < 2:
                return (FAIL_INCORRECT,max(0,score))
        ...
        result = None
        outp.write("Function call rollem(%s,%s) crashed.\n" % repr(FIRST), repr(LAST))
        outp.write(traceback.format_exc(0))
        return (FAIL_CRASHES,0)
    ...
    try:
        ...
        result = None
        outp.write("Function call rollem(%s) crashed.\n" % repr(FIRST))
        outp.write(traceback.format_exc(0))
        return (FAIL_CRASHES,0)
    ...
    if step == 0:
        try:
            env.module.rollem()
            outp.write('Function rollem has default values for BOTH parameters.\n')
            return (FAIL_INCORRECT,max(0,score-0.5))
        except:
            pass
    else:
        try:
            if env.module.rollem() != 5:
                outp.write('Function rollem returns the wrong value.\n')
                score -= 0.35
                if step < 2:
                    return (FAIL_INCORRECT,max(0,score))
            ...
            result = None
            outp.write("Function call rollem() crashed.\n")
            outp.write(traceback.format_exc(0))
            return (FAIL_CRASHES,0)
...
def grade_file(file,outp=sys.stdout):
    ...
    Precondition: outp is a stream writer
    """
    outp.write('Function body comments:\n')
    status, p1 = grade_function(file,1,outp)
    if p1 == 1:
    ...
    if not status:
        outp.write('Specification comments:\n')
        status, p2 = grade_spec(file,1,outp)
        if p2 == 1:
            outp.write('The specification looks good.\n\n')
        else:
            outp.write('\n')
    else:
        p2 = 0
    ...
    if not status:
        outp.write('Optional argument comments:\n')
        status, p3 = grade_optional(file,1,outp)
        if p3 == 1:
            outp.write('The optional arguments looks good.\n\n')
        else:
            outp.write('\n')
    else:
        p3 = 0

    total = round(0.1*p1+0.1*p2+0.8*p3,3)
    return total
