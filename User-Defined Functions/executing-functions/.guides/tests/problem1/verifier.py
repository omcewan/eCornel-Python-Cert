...
def grade_header(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the function header
    ...
    if not env.module.rollem.__doc__:
        outp.write("Procedure rollem does not have a docstring.\n")
        score -= 0.35
        if not step:
        ...
        if (correct != actuals):
            outp.write("Procedure rollem does not have the correct docstring.\n")
            score -= 0.2
            if not step:
                return (FAIL_INCORRECT, max(0,score))
...
def grade_body(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the function body
    
    The step parameter is the phase in the grading pass.  Step 0 only looks for one
    print statement.  Step 1 looks for two print statements.  Step 2 verifies that
    the calculation is correct.  Steps 0-2 will stop at the first error found.  Step 3
    if the final grading pass and will continue through and try to find all errors.
    
    Parameter name: The file name
    Precondition: name is a string
    
    Parameter step: The current verfication/grading step
    Precondition: grade is 0, 1, 2, or 3
    ...
    if type(env) == str:
        ...
        outp.write('File %s has a print statement outside of the function body.\n' % repr(file))
        score -= 0.15
        if step < 3:
            return (FAIL_INCORRECT, max(0,score))
    
    count = 1 if step == 0 else 2
    import random
    random.seed(10)
    ...
    err = check_print(env,FIRST,LAST,6 if count > 1 else None)
    if err:
    ...
    err = check_print(env,FIRST,LAST,7 if count > 1 else None)
    if err:
        if err == PRINT_FIRST_NO_PERIOD:
...
def grade_file(file,outp=sys.stdout):
    ...
    if not status:
        outp.write('Header comments:\n')
        status, p2 = grade_header(file,1,outp)
        if p2 == 1:
            outp.write('The function header looks good.\n\n')
        else:
            outp.write('\n')
    else:
        p2 = 0
    ...
    if not status:
        ...
        status, p3 = grade_body(file,3,outp)
        if p3 == 1:
    ...
    total = round(0.2*p1+0.3*p2+0.5*p3,3)
    return total
...
def grade(outp=sys.stdout):
    ...
    """
    return grade_file('proc.py',outp)
