...
def is_two_words(w):
    ...
    """
    # Slicing works even if string empty
    cond1 = (w[:1] != ' '  and w[:1] != '')
    cond2 = (w[-1:] != ' ' and w[-1:] != '')
    cond3 = ' ' in w
    
    # Find first and last and verify all are spaces
    first = introcs.find_str(w,' ')
    last = introcs.rfind_str(w,' ')
    midl = w[first:last+1]
    cond4 = introcs.count_str(midl,' ') == len(midl)
