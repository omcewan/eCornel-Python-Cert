...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    table1 = [[0.7, 0.0, -0.7, 1.0, -0.9]]
    table2 = [[0.7], [0.1], [0.4], [-0.2], [0.6]]
    table3 = [[0.7, 0.0, -0.7], [0.1, 0.4, -0.6]]
    table4 = [[0.7, 0.0, -0.7, 1.0], 
              [0.1, 0.4, -0.6, 0.9],
              [0.4, -0.1, -0.2, 0.1]]
    table5 = [[0.7, 0.0, -0.7, 1.0, -0.9], 
              [0.1, 0.4, -0.6, 0.9, 0.2],
              [0.4, -0.1, -0.2, 0.1, -0.6], 
              [-0.2, 0.2, 0.2, -0.3, -0.2],
              [0.6, -0.1, -0.0, 0.8, 0.9]]
    
    testcase = [(table1,[0.1]),(table2,[0.7, 0.1, 0.4, -0.2, 0.6]),(table3,[0.0, -0.1]),
                (table4,[1.0, 0.8, 0.2]),(table5,[0.1, 1.0, -0.4, -0.3, 2.2])]
    ...
    for data in testcase:
        ...
        try:
            ...
            compare = False
            try:
                compare = introcs.allclose(received,expected)
            except:
            ...
            if not compare:
                ...
                outp.write("The call %s(%s) modified the list.\n" % (function, repr(brgs)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
