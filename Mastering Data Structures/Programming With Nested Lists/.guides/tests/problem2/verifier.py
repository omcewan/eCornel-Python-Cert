...
def grade_func1(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of place_sums
    ...
    headers = ['P1','S3','Q2','P5','R10','T9']
    table1 = [headers[:5], [0.7, 0.0, -0.7, 1.0, -0.9]]
    table2 = [headers[:1], [0.7], [0.1], [0.4], [-0.2], [0.6]]
    table3 = [headers[:3], [0.7, 0.0, -0.7], [0.1, 0.4, -0.6]]
    table4 = [headers[:4],
              [0.7, 0.0, -0.7, 1.0], 
              [0.1, 0.4, -0.6, 0.9],
              [0.4, -0.1, -0.2, 0.1]]
    ...
    testcase = [(table1,list(map(answer,table1))),
                (table2,list(map(answer,table2))),
                (table3,list(map(answer,table3))),
                (table4,list(map(answer,table4))),
                (table5,list(map(answer,table5)))]
    ...
    for data in testcase:
        ...
        try:
            ...
            compare = False
            try:
                compare = alst[0] == expected[0]
                compare = compare and introcs.allclose(alst[1:],expected[1:])
            except:
                compare = expected == [] and alst == []
            ...
            elif not compare and alst == brgs[0]:
...
def grade_func2(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of crossout
    ...
    table2 = [[0.1,0.3,0.5],[0.6,0.2,0.7],[1.5,2.3,0.4],[0.1,0.2,0.3]]
    table3 = [[0.1,0.3,0.5,1.0],[0.6,0.2,0.7,2.0],[1.5,2.3,0.4,3.0]]
    table4 = [[1,2],[3,4]]
    
    testcase = [(copy.deepcopy(table1),1,2,[[0.1,0.3],[1.5,2.3]]),
                (copy.deepcopy(table1),0,0,[[0.2,0.7],[2.3,0.4]]),
                (copy.deepcopy(table1),2,1,[[0.1,0.5],[0.6,0.7]]),
                (copy.deepcopy(table1),1,0,[[0.3,0.5],[2.3,0.4]]),
                (copy.deepcopy(table2),1,2,[[0.1,0.3],[1.5,2.3],[0.1,0.2]]),
                (copy.deepcopy(table2),0,0,[[0.2,0.7],[2.3,0.4],[0.2,0.3]]),
                (copy.deepcopy(table3),1,2,[[0.1,0.3,1.0],[1.5,2.3,3.0]]),
                (copy.deepcopy(table3),0,0,[[0.2,0.7,2.0],[2.3,0.4,3.0]]),
                (copy.deepcopy(table4),1,0,[[2]]),
                (copy.deepcopy(table4),0,1,[[3]]),
                ([[5]],0,0,[])]
    ...
    for data in testcase:
        ...
        try:
            ...
            compare = False
            try:
                compare = introcs.allclose(alst,expected)
            except:
                compare = expected == [] and alst == []
