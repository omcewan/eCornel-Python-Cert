...
def grade_func1(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of average_grade
    ...
    grades1 = { 'wmw2': 55, 'abc3': 90, 'jms45': 86 }
    grades2 = { 'abc123': 0,'abc456':65,'jms457':50,'jms123':60,'xyz123':70,'xyz456':80,
                'wmw4':90,'wmw5':100,'tor3':88 }
    testcase = [(dict(list(grades1.items())[:2]),72.5),
                (dict(list(grades1.items())[1:2]),90.0),
                (dict(list(grades1.items())[:1]),55.0),
                (dict(list(grades2.items())[:3]),38.333),
                (dict(list(grades2.items())[:5]),49.0),
                (dict(list(grades2.items())[:7]),59.286),
                (dict(list(grades2.items())[2:7]),70.0),
                (dict(list(grades2.items())[:8]),64.375),
                (grades2,67.0),(grades1,77.0),({},0.0)]
    ...
    for data in testcase:
        ...
        try:
            env.reset()
            received = func(args)
            if not type(received) in [int,float] or not introcs.isclose(float(received),expected):
                outp.write("The call %s(%s) returns %s, not %s.\n" % (function, repr(brgs), repr(received), repr(expected)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            elif args != brgs:
                outp.write("The call %s(%s) modified the dictionary.\n" % (function, repr(brgs)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
...
def grade_func2(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of letter_grades
    ...
    grades1 = { 'wmw2': 55, 'abc3': 90, 'jms45': 86 }
    answer1 = { 'wmw2': 'F', 'abc3': 'A', 'jms45': 'B' }
    grades2 = { 'abc123': 0,'abc456':65,'jms457':50,'jms123':60,'xyz123':70,
                'xyz456':80,'wmw4':90,'wmw5':100,'tor3':88 }
    answer2 = { 'abc123':'F','abc456':'D','jms457':'F','jms123':'D','xyz123':'C',
                'xyz456':'B','wmw4':'A','wmw5':'A','tor3':'B'}
    testcase = [(dict(list(grades1.items())[:2]),(dict(list(answer1.items())[:2]))),
                (dict(list(grades1.items())[1:2]),(dict(list(answer1.items())[1:2]))),
                (dict(list(grades1.items())[:1]),(dict(list(answer1.items())[:1]))),
                (dict(list(grades2.items())[:3]),(dict(list(answer2.items())[:3]))),
                (dict(list(grades2.items())[:5]),(dict(list(answer2.items())[:5]))),
                (dict(list(grades2.items())[:7]),(dict(list(answer2.items())[:7]))),
                (dict(list(grades2.items())[2:7]),(dict(list(answer2.items())[2:7]))),
                (dict(list(grades2.items())[:8]),(dict(list(answer2.items())[:8]))),
                (grades2,answer2),(grades1,answer1),({},{})]
