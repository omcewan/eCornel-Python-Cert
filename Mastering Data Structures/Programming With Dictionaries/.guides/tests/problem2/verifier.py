...
def grade_func(file,step=0,outp=sys.stdout):
    ...
    weather1 = introcs.read_json(os.path.join(LOCALSPACE,'weather.json'))
    weather2 = introcs.read_json(os.path.join(LOCALSPACE,'alternate.json'))
    testcase = [(1,20,0),(1,10,1),(1,7,40),(1,5,58),(1,2,116),(1,1,140),(1,-2,258),
                (1,-3,325),(1,-5,392),(1,-10,505),(1,-11,531),(1,-15,600),(1,-20,646),
                (2,20,30),(2,10,103),(2,7,145),(2,5,176),(2,2,381),(2,1,455),(2,-2,581),
                (2,-3,652),(2,-5,719),(2,-10,874),(2,-11,896),(2,-15,959),(2,-20,987)]
    
    func = getattr(env.module,function)
    printed = False
    for data in testcase:
        weather = weather1 if data[0] == 1 else weather2
        arg = data[1]
        expected = data[2]
        try:
            env.reset()
            received = func(weather,arg)
            if received != expected:
                outp.write("The call %s(weather,%s) returns %s, not %s.\n" % (function, repr(arg), repr(received), repr(expected)))
            ...
            import traceback
            outp.write("The call %s(weather,%s) crashed.\n" % (function, repr(arg)))
