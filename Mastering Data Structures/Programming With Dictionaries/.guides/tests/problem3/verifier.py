...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    for data in testcase:
        ...
        try:
            ...
            elif not compare and args == brgs:
                outp.write("The call %s(%s) did not modify the dictionary.\n" % (function, repr(brgs)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            elif not compare:
                outp.write("The call %s(%s) modified the dictionary to %s when %s was expected.\n" % (function, repr(brgs), repr(args), repr(expected)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
...
def grade_func2(file,step=0,outp=sys.stdout):
    ...
    testcase = []
    for thresh in (0,20,55,60,80,86,90,95,100):
        answer1 = dict(filter(lambda x: x[1] >= thresh,grades1.items()))
        testcase.append((dict(grades1),thresh,answer1))
        answer2 = dict(filter(lambda x: x[1] >= thresh,grades2.items()))
        testcase.append((dict(grades2),thresh,answer2))
    ...
    for data in testcase:
        ...
        try:
            env.reset()
            received = func(*args)
            compare  = args[0] == expected 
            if not received is None:
                outp.write("The call %s%s returns %s, not None.\n" % (function, repr(brgs), repr(received)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            elif not compare and args[0] == brgs[0]:
                outp.write("The call %s%s did not modify the dictionary.\n" % (function, repr(brgs)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            elif not compare:
                outp.write("The call %s%s modified the dictionary to %s when %s was expected.\n" % (function, repr(brgs), repr(args[0]), repr(expected)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            if len(env.printed) != 0:
