...
WORKSPACE = [os.path.expanduser('~'),'workspace','project']
#WORKSPACE = []
...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    testcase = [(add,(),3,3),(sub,(),2,2),(add,(4,),0,4),(sub,(4,),0,-4),(sub,(4,),2,-2),
                (add,(1,2,3,4),0,10),(sub,(1,2,3,4),0,-10),(sub,(-1,-2,-3,-4),0,10),
                (add,'','A','A'),(add,'B','','B'),(add,'bcdefg','a','abcdefg'),
                (remove,'','a','a'),(remove,'a','a',''),(remove,'b','a','a'),
                (remove,'abcef','abcefhijklimnop','hijklimnop'),
                (duplicate,'abc','ab','aabb'),
                (duplicate,'abc','xy','xy'),
                (duplicate,'hello','ol','oollll')]
    
    func = getattr(env.module,function)
    printed = False
    for data in testcase:
        args = data[:-1]
        blob = ', '.join((args[0].__name__,)+tuple(map(repr,args[1:])))
        expected = data[-1]
        try:
            env.reset()
            received = func(*args)
            if received != expected:
                outp.write("The call %s(%s) returns %s, not %s.\n" % (function, blob, repr(received), repr(expected)))
...
def grade_func2(file,step=0,outp=sys.stdout):
    ...
    testcase = [(add,(),3,3),(sub,(),2,2),(add,(4,),0,4),(sub,(4,),0,4),(sub,(4,),2,2),
                (add,(1,2,3,4),0,10),(sub,(1,2,3,4),0,-2),(sub,(-1,-2,-3,-4),0,2),
                (add,'','A','A'),(add,'B','','B'),(add,'bcdefg','a','bcdefga'),
                (remove,'','a','a'),(remove,'a','a',''),(remove,'b','a','b'),
                (remove,'abc','d','a'),(remove,'aabc','d',''),
                (duplicate,'abc','ab','a'),
                (duplicate,'abc','xy','a'),
                (duplicate,'hello','ol','h')]
