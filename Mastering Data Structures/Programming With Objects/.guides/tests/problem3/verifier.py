...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    testcase = [('A (B) C','B'), ('A (B) C (','B'), ('A (B) C )','B'), ('A (B) (C) D','B'),
...
def grade_func2(file,step=0,outp=sys.stdout):
    ...
    good = ['wmw2','jrs1234','ww9999','qq1']
    bads = ['Wmw2','wMw2','wmW2','wmw-2','ww99a99','#w999','w#w999','ww#999',' ww9'] 
...
def grade_structure(file,func,step=0,outp=sys.stdout):
    ...
    for item in code.body:
        if type(item) == ast.FunctionDef and item.name == func:
            ...
            for n in item.names:
                alias = n.name.split('.')
                imports.extend(alias)
        elif type(item) == ast.ImportFrom:
            alias = item.module.split('.')
            imports.append(alias)
    
    if 'introcs' in imports:
        outp.write("File %s should not import 'introcs' or any function from it.\n" % repr(file))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT,max(0,score))
    ...
    if 'introcs' in imports:
        outp.write("Function %s should not import 'introcs' or any function from it.\n" % repr(func))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT,max(0,score))
    elif found2:
