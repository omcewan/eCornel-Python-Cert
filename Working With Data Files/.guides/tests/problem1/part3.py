...
class HTMLizer(object):
    ...
    This class ensures that all writes to the output are HTML safe.
    """
...
def check_func3(file):
    """
    Checks that the file is correct
    
    Parameter file: The file to check
    Precondition: file is a string
    """
    outp = HTMLizer()
    result = verifier.grade_docstring(file,0,outp)
    if not result[0]:
        result = verifier.grade_func1(file,0,outp)
    if not result[0]:
        result = verifier.grade_func2(file,0,outp)
    if not result[0]:
        print("Both functions look correct.")
    return result[0]
