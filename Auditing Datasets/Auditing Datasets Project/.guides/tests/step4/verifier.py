...
def check_execute_error(lines,value):
    ...
    if len(lines) == 0:
        ...
        message = 'execute(%s) printed more than one line. Output:\n\n%s' % (repr(value), "\n".join(lines))
...
def grade_func1(package,module,step=0,outp=sys.stdout):
    ...
    try:
        ...
        if not env.printed:
            ...
            outp.write("%s(%s,None) printed more than one line. Output:\n\n%s\n" % (function,npath,"\n".join(env.printed)))
            score -= 0.2
            if not step:
                return (FAIL_INCORRECT, max(0,score))
    ...
    # Clean up any old output file STT
    try:
        os.remove(opath)
    except:
        pass
    ...
    try:
        ...
        if not env.printed:
            ...
            outp.write("%s(%s,%s) printed more than one line. Output:\n\n%s\n" % (function,npath,repr(opath),"\n".join(env.printed)))
            score -= 0.1
            if not step:
                return (FAIL_INCORRECT, max(0,score))
        elif env.printed[0].strip() == expect[:-1]:
        ...
        if not os.path.exists(opath):
            outp.write("%s(%s,%s) did not create the file %s.\n" % (function,npath,repr(opath),repr(opath)))
            score -= 0.5
            if not step:
                return (FAIL_INCORRECT, max(0,score))
        ...
        if data is None:
            ...
            if header2 != header1:
                outp.write('The header for %s should be %s, not %s.\n' % (repr(opath),header1,header2))
                score -= 0.1
                if not step:
                    return (FAIL_INCORRECT, max(0,score))
