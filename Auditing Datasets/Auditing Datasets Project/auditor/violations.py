    ...
    if visibility == "unavailable":
        ...
        if "minimum" in visibility.keys():
            if visibility['units'] == "SM":
                ...
                visibility_miles = visibility['minimum']/5280
                if visibility_miles < minimum:
                    return True
                else:
                    return False
        else:
            if visibility['units'] == "SM":
                ...
                visibility_miles = visibility['prevailing']/5280
                if visibility_miles < minimum:
                    return True
                else:
                    return False
...
def bad_winds(winds, maxwind, maxcross):
    ...
    if "speed" in winds.keys() and 'gusts' in winds.keys():
        if winds["units"] == 'KT':
            if winds["speed"] > winds["gusts"]:
                if winds["speed"] > maxwind:
                    return True
                else:
                    if "crosswind" in winds.keys():
            ...
            speed_knots = winds["speed"] * 1.94384
            gusts_knots = winds["gusts"] * 1.94384
            if speed_knots > gusts_knots:
                if speed_knots > maxwind:
                    return True
                else:
                    if "crosswind" in winds.keys():
                        crosswind_knots = winds['crosswind'] * 1.94384
                        if crosswind_knots > maxcross:
                            return True
                        else:
                            return False
                    else:
...
def bad_ceiling(ceiling, minimum):
    ...
    for layer in range(len(ceiling)):
        if ceiling[layer]['type'] == 'broken' or ceiling[layer]['type'] == 'overcast' or ceiling[layer]['type'] == 'indefinite ceiling':
            if ceiling[layer]['height'] < minimum:
                return True
            else:
                return False
...
def get_weather_report(takeoff, weather):
    ...
    iso_str = str(takeoff.isoformat())

    if iso_str in weather.keys():
        return weather[iso_str]

    weather_keys = []

    for key in weather.keys():
        key_datetime = utils.str_to_time(key)
        if key_datetime.date() == takeoff.date():
            if key_datetime.time() < takeoff.time():
                weather_keys.append(key)

    weather_keys.sort()
    return weather[weather_keys[-1]]
...
def list_weather_violations(directory):
    ...
    # Get the takeoff time x
    # Get the pilot credentials x
    # Get the pilot minimums x
    ...
    weather = utils.read_json(directory + '/' + WEATHER)
    minimums = utils.read_csv(directory + '/' + MINIMUMS)
    students = utils.read_csv(directory + '/' + STUDENTS)
    
    # store violated flights
    violated_flights = []

    for row in range(1, len(lessons)):
        # get the takeoff time as a datetime as we will need to pass it into other helper functions
        takeoff_time = utils.str_to_time(lessons[row][3])

        # will hold the information for a student once we find the row corresponding to student id
        student_info = utils.get_for_id(lessons[row][0], students)

        # set instructed to false and then we use a if statement to check for a value within lessons column for instructed and reassign if it Truthy
        instructed = True
        if not lessons[row][2]:
            instructed = False
        
        # set the boolean value for vfr
        vfr = True
        if lessons[row][5] == 'IFR':
            vfr = False
            
        # get the creditials for the specific pilot
        pilot_creds = pilots.get_certification(takeoff_time, student_info)

        # we set day to try and then use a conditions to change it based on the return value of the function utils.daytime
        day = True
        if not utils.daytime(takeoff_time, daycycle):
            day = False

        # gets the pilot minimus
        pilot_min = pilots.get_minimums(
            pilot_creds, lessons[row][6], instructed, vfr, day, minimums)

        if pilot_min:
            # get the weather report for this time
            weather_report = get_weather_report(takeoff_time, weather)

            # get the weather violation to attach to the end of the row if one is found
            weather_violations = get_weather_violation(weather_report, pilot_min)
            
            # if a violation is found append to the row and then append the row to the list violated flights
            if weather_violations:
                lessons[row].append(weather_violations)
                violated_flights.append(lessons[row])

    return violated_flights
