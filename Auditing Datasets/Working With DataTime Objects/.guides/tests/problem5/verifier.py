...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    import ast # Unclear why ast needs to be imported again, but line below fails with UndefinedLocalVariable otherwise
    ...
    if afunc:
        ...
        if atry:
            ...
            if len(atry.body) > 1:
                ...
                outp.write("(Otherwise unexpected errors in these other lines of code will be 'hidden' in the try clause and thus be harder to identify.)\n")
                score -= 0.5
                if not step:
