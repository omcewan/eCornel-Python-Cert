...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    testcases = [(2019,3),(2018,2),(2017,1),(2016,7),(1984,2),(2100,6)]
    ...
    for data in testcases:
        try:
            ...
            expected = data[1]
            if type(received) != int or received < 1 or received > 7:
                outp.write("The call %s(%s) does not return an int in range 1..7.\n" % (function, repr(data[0])))
...
def grade_func2(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of print_iso
    ...
    testcases = [(date(2019,10,12),time(12,35,15,205),'2019-10-12T12:35:15.000205'),
                 (date(2019,10,12),time(9,15),'2019-10-12T09:15:00'),
                 (date(2019,10,12),time(23,59,59),'2019-10-12T23:59:59'),
                 (date(1984,6,5),time(12,35,15,205),'1984-06-05T12:35:15.000205'),
                 (date(1984,6,5),time(9,15),'1984-06-05T09:15:00'),
                 (date(1984,6,5),time(23,59,59),'1984-06-05T23:59:59')]
    func = getattr(env.module,function)
    printed = False
    for data in testcases:
        try:
            ...
            if type(received) != str:
                outp.write("The call %s%s does not return a string.\n" % (function, repr(data[:2])))
                score -= 1/len(testcases)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            if received != expected:
