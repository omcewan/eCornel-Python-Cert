...
def grade_func(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the implementation of write_csv
    ...
    # Check for a file mode other than "w" in the call to open() (using ast analysis) STT
    tree = parse_file("func.py")
    for node in ast.walk(tree):
        # Searching for a Call node with a Name func attribute and a function name of "open"
        if type(node) == ast.Call and type(node.func) == ast.Name and  node.func.id == 'open':
            # Check for at least 2 args, the second arg being Str and having a mode value of "a"
            if len(node.args) >= 2 and type(node.args[1]) == ast.Str and node.args[1].s == 'a':
                outp.write("You opened the file using append mode instead of writing mode in %s.\n" % repr(function))
                score -= 0.5
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
