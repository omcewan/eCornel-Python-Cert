...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    testcases = [('readfile1.txt',6),('readfile2.txt',23),('readfile3.txt',10)]
...
def grade_func2(file,step=0,outp=sys.stdout):
    ...
    testcases = [(5,'writefile1.txt'),(16,'writefile2.txt'),(26,'writefile3.txt')]
    func = getattr(env.module,function)
    printed = False
    unclosed = False
    unmatched = False
    for data in testcases:
        innpath = os.path.join(*TESTFILES,data[1])
        outpath = os.path.join(*TESTFILES,'temp'+str(data[0])+'.txt')
        try:
            env.reset()
            func(outpath,data[0])
            if not os.path.exists(outpath):
                outp.write("The call %s(%s,%s) did not create a file.\n" % (function, repr(outpath), repr(data[0])))
                score -= 1/len(testcases)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            else:
